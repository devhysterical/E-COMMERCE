...
const LowStockAlert = ({
  ...
  if (isLoading) {
    return (
      <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded-2xl p-4 animate-pulse">
        <div className="h-6 bg-amber-200 dark:bg-amber-800 rounded w-48 mb-3"></div>
        <div className="h-4 bg-amber-100 dark:bg-amber-900/40 rounded w-32"></div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/40 dark:to-orange-950/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-5 shadow-sm">
      {/* Header */}
        ...
        <div className="mb-3">
          ...
          <div className="space-y-2">
            {criticalProducts.slice(0, 3).map((product) => (
              <div
                key={product.id}
                className="flex items-center gap-3 bg-white/70 dark:bg-slate-900/70 rounded-xl p-2.5 border border-red-100 dark:border-red-900/50">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {lowProducts.slice(0, 5).map((product) => (
              <div
                key={product.id}
                className="flex items-center gap-2 bg-white/70 dark:bg-slate-900/70 rounded-lg px-2.5 py-1.5 border border-amber-100 dark:border-amber-900/50 text-sm">
                <span className="text-slate-700 dark:text-slate-200 truncate max-w-32">
                  {product.name}
                </span>
