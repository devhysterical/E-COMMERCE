        ...
        <>
          <div className="grid gap-4">
            {banners.slice((page - 1) * limit, page * limit).map((banner) => (
              <div
                key={banner.id}
                className={`flex items-center gap-4 p-4 bg-white dark:bg-slate-900 rounded-xl border ${
                  banner.isActive
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            {formData.imageUrl ? (
              ...
              <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-lg cursor-pointer hover:border-indigo-500 transition-colors">
                <Upload
                  size={32}
                  className={`${
