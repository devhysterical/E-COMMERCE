...
describe('ProductsService', () => {
  ...
  const mockProduct = {
    ...
    categoryId: 'cat-1',
    deletedAt: null,
    category: { id: 'cat-1', name: 'Thời trang' },
    reviews: [],
    images: [],
  };
  ...
  describe('update', () => {
    it('should update a product', async () => {
      ...
      const result = await service.update('prod-1', { name: 'Áo thun mới' });
  ...
  describe('remove', () => {
    it('should soft-delete a product', async () => {
      ...
      const calls = mockPrisma.product.update.mock.calls as Array<
        [{ data: { deletedAt: Date } }]
      >;
  ...
  describe('searchSuggest', () => {
    it('should return search suggestions', async () => {
      mockPrisma.product.findMany.mockResolvedValue([
        { id: 'prod-1', name: 'Áo thun', price: 150000, imageUrl: null },
      ]);
  ...
  describe('Image Management', () => {
    ...
    it('removeImage should throw if image not found', async () => {
      ...
      await expect(
        service.removeImage('prod-1', 'nonexistent'),
      ).rejects.toThrow(NotFoundException);
    });

    it('setPrimaryImage should update primary flag', async () => {
      const img = { id: 'img-1', productId: 'prod-1', isPrimary: false };
      mockPrisma.productImage.findFirst.mockResolvedValue(img);
      mockPrisma.productImage.updateMany.mockResolvedValue({ count: 2 });
