...
describe('ContactService', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        service = module.get(_contactservice.ContactService);
        jest.clearAllMocks();
    });
    const mockTicket = {
        id: 'ticket-1',
    ...
    describe('create', ()=>{
        it('should create a ticket and send email notification', async ()=>{
            ...
            mockEmailService.sendContactNotificationEmail.mockResolvedValue(undefined);
            ...
            expect(mockEmailService.sendContactNotificationEmail).toHaveBeenCalledWith(mockTicket);
        });
        it('should still succeed if email notification fails', async ()=>{
            mockPrisma.contactTicket.create.mockResolvedValue(mockTicket);
            mockEmailService.sendContactNotificationEmail.mockRejectedValue(new Error('SMTP error'));
            const result = await service.create({
                name: 'Nguyễn Văn A',
                email: 'test@example.com',
                subject: 'Hỏi',
    ...
    describe('findAll', ()=>{
        ...
        it('should filter by status', async ()=>{
            ...
            expect(mockPrisma.contactTicket.findMany).toHaveBeenCalledWith(expect.objectContaining({
                where: {
                    status: 'OPEN'
                }
            }));
